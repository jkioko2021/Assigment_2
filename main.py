...
# Open our model
model = pickle.load(open('model/model_2.pkl', 'rb'))
...
def predict():
    ...
    if output < 0:
        return render_template('index.html',
                               prediction_text="Predicted a negative  value, values entered not reasonable")
    elif output >= 0:
        return render_template('index.html', prediction_text='Predicted : {}'.format(output))
